...
fn test_proxy_pattern() {
    ...
    let method = common::create_test_method(
        target_sel.clone(),
        common::void_method_impl,
    );
    ...
    *FORWARDING_TARGET.write().unwrap() = Some(target_obj.clone());
    ...
    let result =
        Object::send_message(&source_obj, &target_sel, &MessageArgs::None);
...
fn test_delegation_pattern() {
    ...
    let method =
        common::create_test_method(sel.clone(), common::counter_method_impl);
    ...
    *DELEGATE.write().unwrap() = Some(delegate_obj.clone());
    ...
    delegator_class
        .set_forwarding_hook(|_obj, _sel| DELEGATE.read().unwrap().clone());
    ...
    assert_eq!(
        common::get_call_count(),
        1,
        "Delegate method should be called once"
    );
...
fn test_dynamic_scripting_object() {
    ...
    let handler_method = common::create_test_method(
        handler_sel.clone(),
        common::void_method_impl,
    );
...
fn test_diagnostic_logging() {
    ...
    let method =
        common::create_test_method(sel.clone(), common::void_method_impl);
    target_class.add_method(method).unwrap();
    let target_obj = Object::new(&target_class).unwrap();
    ...
    *TARGET.write().unwrap() = Some(target_obj);
    ...
    source_class
        .set_forwarding_hook(|_obj, _sel| TARGET.read().unwrap().clone());
